...
EXEMPLO DE COMO UTILIZAR NOS METODOS

		context_email = {}
		context_email['nome'] = request.user
		template = render_to_string('email/boletim/boletins.html',context_email)


		EmailThread('Arvore Consultoria',
			'',
			'Arvore Consultoria',
			['yelken@3ysoftwarehouse.com.br', 'ytalo@3ysoftwarehouse.com.br', 'lfa.luisfelipe@gmail.com'],
			False,
			template).start()
'''
...
class Dashboard(JSONResponseMixin, View):
	def get(self, request):
		...
		tenbefore = datetime.now() - timedelta(days = 11)
		twentybefore = datetime.now() - timedelta(days = 21)
		thirtybefore = datetime.now() - timedelta(days = 31)
		fortybefore = datetime.now() - timedelta(days = 41)

		lessthanten  = Boletim.objects.filter(data_procedimentoinicio__gte = tenbefore).filter(data_entrada__isnull = True).count()
		between = Boletim.objects.filter(data_procedimentoinicio__lte = tenbefore).filter(data_procedimentoinicio__gte = twentybefore).filter(data_entrada__isnull = True).count()
		greatherthantwenty = Boletim.objects.filter(data_procedimentoinicio__lte = twentybefore).filter(data_procedimentoinicio__gte = thirtybefore).filter(data_entrada__isnull = True).count()

		greatherthanthirty = Boletim.objects.filter(data_procedimentoinicio__lte = thirtybefore).filter(data_procedimentoinicio__gte = fortybefore).filter(data_entrada__isnull = True).count()
		greatherthanforty = Boletim.objects.filter(data_procedimentoinicio__lte = fortybefore).filter(data_entrada__isnull = True).count()
		delivered = Boletim.objects.filter(data_entrada__isnull = False).count()
		todos = Boletim.objects.all().count()
		naopreenchido = Boletim.objects.filter(id_status_boletim__isnull=True).filter(id_hospital__isnull=True).filter(id_convenio__isnull=True).filter(id_pagador__isnull=True).filter(id_acomodacao__isnull=True).count()
		
		reembolsosreceber = 0
		reembolsosrecebido = 0
		reembolsogeral = 0

		for value in Reembolso.objects.all():
			if not value.valor_pago:
				reembolsosreceber += 1
				if value.valor_servico:
					reembolsogeral += value.valor_servico
			elif value.valor_pago == value.valor_servico:
				reembolsosrecebido += value.valor_servico
			else:
				if value.valor_pago < value.valor_servico:
					reembolsosreceber += 1
					reembolsogeral += value.valor_servico - value.valor_pago
		

		response = {}
		response['lessthanten'] = lessthanten
		response['between'] = between
		response['greatherthantwenty'] = greatherthantwenty

		response['greatherthanthirty'] = greatherthanthirty

		response['greatherthanforty'] = greatherthanforty

		response['delivered'] = delivered
		response['all'] = todos
		response['naopreenchido'] = naopreenchido
		response['reembolsosreceber'] = reembolsosreceber
		response['reembolsogeral'] = reembolsogeral
		response['reembolsosrecebido'] = reembolsosrecebido
		return render (request, 'default/dashboard.html', {'contagem_boletins': response})
